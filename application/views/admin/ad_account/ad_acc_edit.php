...
<?php echo form_open_multipart('account/',array('id' => 'cmsForm','autocomplete' => 'off') ); ?>
...
<div class="row">
     ...
     <?php 
        $file_url= base_url('assets/dist-assets/images/faces/user.jpg');

        $file_name= "";
        if($user['profile_picture']){
            $file_name = $user['profile_picture'];
            $file_url = base_url($user['profile_picture']);
        }
    ?>
...
<div class="row">
<div class="col-md-6">
<div class="mt-2 mb-2 form-group"><label for=" ">First Name </label><input class="form-control"  type="text" value="<?php echo $user['first_name']; ?>" name="first_name"></div>
</div>

<div class="col-md-6">
<div class="mt-2 mb-2 form-group"><label for=" ">Last Name</label><input class="form-control" type="text" value="<?php echo $user['last_name']; ?>"  name="last_name"></div>
</div>

<div class="col-md-6">
<div class="mt-2 mb-2 form-group"><label for=" ">Email </label><input class="form-control"  type="text" value="<?php echo $user['email']; ?>" readonly=""></div>
</div>
...
<div class="col-md-6">
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            <select class="form-control form-control-rounded" id="country_code" name="country_code">
                ...
                <?php
                    ...
                    foreach ($phonecodes as $key => $value) {
                        if($value['phonecode']){
                             $attr = '';
                            if($value['phonecode'] == $user['country_code']){
                                $attr = 'selected';
                            }
                            echo '<option value="'.$value['phonecode'].'" '.$attr.' >'.$value['phonecode'].'</option>';
                        }
                    }
                ?>
            </select>
         ...
         <div class="col-md-8">
            <div class="mt-2 mb-2 form-group">
                <label for=" ">Contact Number </label><input class="form-control" type="text" value="<?php echo $user['mobile']; ?>" name="mobile">
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function(){
	...
	$('#cmsForm').validate({
        ...
        rules: {
            ...
            last_name: {required: false, lettersonly:true},
